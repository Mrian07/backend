    ...
    if (isset( $session )) {		// print_r( $session ); 
		try{
			...
			if($_SESSION['fb_user'] == 'rider') {
				...
				if($femail != '' || $fbid != '') {
					$sql = "SELECT iUserId,vImgName FROM register_user WHERE $sqll001 and eStatus != 'Deleted'";
				...
				if($femail != '' || $fbid != '') {
					$sql = "SELECT iDriverId,vImage FROM register_driver WHERE $sqll001 and eStatus != 'Deleted'";
					$db_user = $obj->MySQLSelect($sql);
				}
