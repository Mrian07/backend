    ...
    if (isset( $session )) {		// print_r( $session ); 
		try{
			...
			if($_SESSION['fb_user'] == 'rider') {
				...
				if($femail != '' || $fbid != '') {
					$sql = "SELECT iUserId,vImgName,vPhone,eGender FROM register_user WHERE $sqll001 and eStatus != 'Deleted'";
				...
				if($femail != '' || $fbid != '') {
					$sql = "SELECT iDriverId,vImage,eGender,vPhone FROM register_driver WHERE $sqll001 and eStatus != 'Deleted'";
					$db_user = $obj->MySQLSelect($sql);
				}
				...
				if(count($db_user) > 0){
					...
					$dRefDate  = Date('Y-m-d H:i:s');	
					if($femail != "") {
						$sql = "INSERT INTO register_driver (vFbId,vName, vLastName, vEmail, iCompanyId, eStatus,vImage,eGender,vLang,vCurrencyDriver,vRefCode,dRefDate) VALUES ('".$fbid."','".$fbfirstname."', '".$fblastname."', '".$femail."', '1', 'Active','','".$fbgender."','".$lang."','".$curr."','".$refercode."','".$dRefDate."')";
