   ...
   for($i=0;$i<count($data_LanguageLabel);$i++){
      ...
      if(count($data_OtherLanguageValue)>0){
          for($j=0;$j<count($data_OtherLanguageValue);$j++){
                ...
                $updateQuery = "UPDATE language_label SET vValue ='".mysql_real_escape_string($getText->translatedText)."', eScript = 'Yes' WHERE LanguageLabelId=".$data_OtherLanguageValue[$j]['LanguageLabelId'];
