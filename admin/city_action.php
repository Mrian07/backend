	...
	if($id != '') {
		...
		if(count($db_data) > 0) {
			foreach($db_data as $key => $value) {
				...
				$vCity	 = $value['vCity'];
				echo $country_id	 = $value['iCountryId'];
				echo $state_id	 = $value['iStateId'];
				$eStatus = $value['eStatus'];
			}
		}
	}
		
	$sql_country = "SELECT iCountryId,vCountry FROM country";
	$db_data_country = $obj->MySQLSelect($sql_country);

	$sql_state = "SELECT iStateId,vState FROM state where iCountryId='".$country_id."'";
