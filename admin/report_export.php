...
//    echo "<pre>";print_r($db_trip); exit;
    ...
    $data .= "\t\t\t\t\t\t\t\t\tTotal Wallet Debit\t" . $generalobj->trip_currency($tot_wallentPayment) . "\n";
    if ($ENABLE_TIP_MODULE == "Yes") {
        $data .= "\t\t\t\t\t\t\t\t\tTotal Driver Payment\t" . $generalobj->trip_currency($total_tip) . "\n";
        $data .= "\t\t\t\t\t\t\t\t\tTotal Driver Payment\t" . $generalobj->trip_currency($tot_driver_refund+$total_tip) . "\n";
    }else {
        $data .= "\t\t\t\t\t\t\t\t\tTotal Driver Payment\t" . $generalobj->trip_currency($tot_driver_refund) . "\n";
    }
	...
	if ($sortby == 4) {
		...
		if ($vEmail != '') {
			$ssql .= " AND rd.vEmail = '" . $vEmail . "'";
				...
				header("Content-Disposition: attachment; filename=$section == 'driver_log_report'.xls");
				header("Pragma: no-cache");
				header("Expires: 0");
				print "$header\n$data";
				exit;
...
if($action == 'search')
{
	...
	if($serachTripNo!=''){
		$ssql.=" AND t.vRideNo ='".$serachTripNo."'";
				...
				$sql_admin = "SELECT t.tStartDate ,t.tEndDate,t.eCancelled,t.vCancelReason,t.vCancelComment,d.iDriverId, t.tSaddress,t.vRideNo,
				t.tDaddress, t.fWalletDebit,t.eCarType,t.iTripId,t.iActive ,CONCAT(d.vName,' ',d.vLastName) AS dName FROM trips t 
			...
			for ($j = 0; $j < count($db_dlip); $j++) {
					...
					$data .= $generalobjAdmin->DateTime($db_dlip[$j]['tStartDate'],'no'). "\t";
					$data .= $CanceledBy. "\t";
					$data .= $trip_cancel . "\t";
				...
				ob_clean();
				header("Content-type: application/octet-sdleam; charset=utf-8");
				header("Content-Disposition: attachment; filename=$section == 'cancelled_trip'.xls");
				header("Pragma: no-cache");
				header("Expires: 0");
				print "$header\n$data";
				exit;
...
if ($section == 'ride_acceptance_report') {
		...
		if ($iDriverId != '') {
			$ssql .= " AND rd.iDriverId = '".$iDriverId."'";
				...
				$header .= "Driver Name" . "\t";
				$header .= "Total Trip Requests". "\t";
				$header .= "Requests Decline" . "\t";
				$header .= "Requests Timeout" . "\t";
				$header .= "Trips Accepted" . "\t";
				$header .= "Trips Finished" . "\t";
			...
			for ($j = 0; $j < count($db_dlip); $j++) {
					...
					$data .= $Cancel."\t";
					$data .= $Finish."\t";
					$data .= round($aceptance_percentage).' %'."\n";
					
				}
				
				ob_clean();
				header("Content-type: application/octet-sdleam; charset=utf-8");
				header("Content-Disposition: attachment; filename=$section == 'ride_acceptance_report'.xls");
				header("Pragma: no-cache");
				header("Expires: 0");
				print "$header\n$data";
				exit;
		...
		if($serachTripNo!=''){
			$ssql.=" AND t.vRideNo ='".$serachTripNo."'";
			 ...
			 for ($j = 0; $j < count($db_dlip); $j++) {
				...
				ob_clean();
				header("Content-type: application/octet-sdleam; charset=utf-8");
				header("Content-Disposition: attachment; filename=$section == 'driver_trip_detail'.xls");
				header("Pragma: no-cache");
				header("Expires: 0");
				print "$header\n$data";
				exit;
