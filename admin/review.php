...
$sql = "SELECT count(r.iRatingId) as Total
			...
			WHERE eUserType='".$chkusertype."' $ssql $adm_ssql";
...
else
{
 ...
 $sql = "SELECT r.iRatingId,r.iTripId,r.vRating1,r.tDate,r.eUserType,r.vMessage,CONCAT(rd.vName,' ',rd.vLastName) as driverName ,rd.vAvgRating,CONCAT(ru.vName,' ',ru.vLastName) as passangerName,ru.vAvgRating as passangerrate,t.iDriverId,t.iUserId,t.vRideNo 
			...
			LEFT JOIN register_driver as rd ON rd.iDriverId=t.iDriverId 
			LEFT JOIN register_user as ru ON ru.iUserId=t.iUserId 
			WHERE 1=1 AND r.eUserType='".$chkusertype."'  $ssql $adm_ssql $ord LIMIT $start, $per_page";
                              ...
                              <tbody>
                                <tr>
                                    ...
                                    <td width="10%" class=" padding-right10"><select name="option" id="option" class="form-control">
                                          ...
                                          <option value="CONCAT(ru.vName,' ',ru.vLastName)" <?php if ($option == "CONCAT(ru.vName,' ',ru.vLastName)") {echo "selected"; } ?> >Rider Name</option>
											...
											<table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                                    ...
                                                    <tr class="gradeA">
																 ...
																 <?php if($reviewtype=='Driver'){?>
                                                                      ...
                                                                      <td><? echo $generalobjAdmin->clearName($data_drv[$i]['driverName']);
																		  echo " <b>( ".$data_drv[$i]['vAvgRating']." )</b>";
																		  ?></td>
																	  
                                                                      <td><? echo $generalobjAdmin->clearName($data_drv[$i]['passangerName']); ?></td>
																	  <?php }else{?>
																	  <td><? echo $generalobjAdmin->clearName($data_drv[$i]['passangerName']); 
																		   echo " <b>( ".$data_drv[$i]['passangerrate']." )</b>";
																		  ?></td>
																	   <td><? echo $generalobjAdmin->clearName($data_drv[$i]['driverName']); ?></td>
