			...
			<div id="content">
				...
				<!-- 
					...
					<div class="panel panel-primary bg-gray-light">
							...
							<div class="col-lg-6">
								<h3><?php echo $langage_lbl_admin['LBL_RIDES_NAME_ADMIN'];?>  Count : <?=($totalRides[0]['tot_trip']);?></h3>
								<p>Today : <b>
								<?
									$today = $generalobjDashboard->getTripDateStates('today');
									echo count($today);
								?></b></p>
								<p>This Month : <b><?=count($generalobjDashboard->getTripDateStates('month'));?></b></p>
								<p>This Year : <b><?=count($generalobjDashboard->getTripDateStates('year'));?></b></p>
								<br />
								<br />
								<p>
					...
					<div class="panel panel-primary bg-gray-light">
							...
							<div class="col-lg-6">
								<h3><?php echo $langage_lbl_admin['LBL_DRIVERS_NAME_ADMIN'];?>  Count : <?=$driver[0]['tot_driver'];?></h3>
					...
					<div class="panel panel-primary bg-gray-light">
							...
							<div class="panel-heading">
								<div class="panel-title-box">
								   <i class="fa fa-bar-chart"></i> Driver Log Report (In minutes)
								</div>                                  
							</div>
...
<script>
	$(document).ready(function(){
			...
			var del_drive = '<?=$delDrive[0]['tot_driver'];?>';

			var total_drive = '<?=$driver[0]['tot_driver'];?>';
			//var total_drive = active_drive + inactive_drive + del_drive;
			...
			Morris.Donut({
				...
				data: [
					...
					{label: "Deleted <?=$langage_lbl_admin['LBL_DRIVERS_NAME_ADMIN'];?>", value: del_drive},
				],
...
<script>
	$(document).ready(function(){
			...
			Morris.Bar({
			  ...
			  data: [
			  //totalTimecount
			    { y: '<?=$generalobjAdmin->clearName($tmp_log_report[0]['Name'])?>', a: <?=$tmp_log_report[0]['totalTimecount']?>},
			    { y: '<?=$generalobjAdmin->clearName($tmp_log_report[1]['Name'])?>', a: <?=$tmp_log_report[1]['totalTimecount']?>},
				{ y: '<?=$generalobjAdmin->clearName($tmp_log_report[2]['Name'])?>', a: <?=$tmp_log_report[2]['totalTimecount']?>},
				{ y: '<?=$generalobjAdmin->clearName($tmp_log_report[3]['Name'])?>', a: <?=$tmp_log_report[3]['totalTimecount']?>},
				{ y: '<?=$generalobjAdmin->clearName($tmp_log_report[4]['Name'])?>', a: <?=$tmp_log_report[4]['totalTimecount']?>}
			  ],
			  xkey: 'y',
			  gridTextColor: '#ee3324',
			  ykeys: ['a'],
