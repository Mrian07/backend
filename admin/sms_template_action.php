<?
	...
	$vSMS_Code = isset($_REQUEST['vSMS_Code'])?$_REQUEST['vSMS_Code']:'';
	...
	$iSMSId = isset($_POST['iSMSId'])?$_POST['iSMSId']:$id;
	...
	if($count_all > 0) {
		for($i=0;$i<$count_all;$i++) {
			$vSubject = 'vBody_'.$db_master[$i]['vCode']; 
	...
	if(isset($_POST['submit'])) {
		if(SITE_TYPE=='Demo')
		{
				header("Location:sms_template_action.php?id=".$iSMSId.'&success=2');
				exit;
		}
		...
		if(count($db_master) > 0) {
			...
			for($i=0;$i<count($db_master);$i++) {
				...
				$str .= " ".$vStatus." = '".$eStatus."', ".$vBody." = '".$$vBody."', ";
			}
		...
		if($action == 'Add')
		{
			$iSMSId =  mysql_insert_id();
		}

		//header("Location:sms_template_action.php?id=".$iSMSId.'&success=1');
		if ($action == "Add") {
            $_SESSION['success'] = '1';
	...
	// for Edit
	if($action == 'Edit') {
		$sql = "SELECT * FROM ".$tbl_name." WHERE iSMSId = '".$id."'";
					...
					<div class="body-div">
						<div class="form-group">
							...
							<form method="post" name="_email_template_form" id="_email_template_form" action=""  enctype="multipart/form-data">
								...
								<input type="hidden" name="vSMS_Code" id="vSMS_Code" value="<?=$vSMS_Code;?>">
