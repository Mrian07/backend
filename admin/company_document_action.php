...
if ($action='document' && isset($_POST['doc_type'])) {
    ...
    if ($image_name != "") {
        ...
        if ($flag_error == 1) {
            ...
            //$generalobj->getPostForm($_POST, $var_msg, "company_document_action.php?success=1&id=" . $_REQUEST['id'] . "&var_msg=" . $var_msg);

            exit;
        } /* else if($_REQUEST['id'] != '') {
          header("location:company_document_action.php?success=0&var_msg=something went wrong. Try again");
          ...
          exit;
            ...
            $query =" INSERT INTO `".$tbl."` ( `doc_masterid`, `doc_usertype`, `doc_userid`, `ex_date`, `doc_file`, `status`, `edate`) "
                   . "VALUES "
                   . "( '".$_REQUEST['doc_type']."', 'company', '".$_REQUEST['id']."', '".$expDate."', '".$vImage."', 'Inactive', CURRENT_TIMESTAMP)";
                ...
                <div class="inner">
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <a href="company.php?type=<? echo $_REQUEST['type']?>">
                                ...
                                <?php for ($i = 0; $i < $count_all; $i++) {  ?>
                                        ...
                                        <div class="panel panel-default upload-clicking">
                                            ...
                                            <div class="panel-body">
                                                <?php if ($iCompanyIddoc[$i]['doc_file'] != '') { ?>
                                                    <?php
                                                    $file_ext = $generalobj->file_ext($iCompanyIddoc[$i]['doc_file']);
                                                    if ($file_ext == 'is_image') {
                                                        ?>
                                                        <img src = "<?= $tconfig["tsite_upload_compnay_doc"] . '/' . $_REQUEST['id'] . '/' . $iCompanyIddoc[$i]['doc_file'] ?>" style="width:200px;cursor:pointer;" alt ="YOUR DRIVING LICENCE" data-toggle="modal" data-target="#myModallicence"/>
                                                    <?php } else { ?>
                                                        <a href="<?= $tconfig["tsite_upload_compnay_doc"] . '/' . $_REQUEST['id'] . '/' . $iCompanyIddoc[$i]['doc_file'] ?>" target="_blank"><?php echo $iCompanyIddoc[$i]['doc_name']; ?></a>
                                                    <?php } ?>
                                                    <?php
                                                } else {
                                                    echo $iCompanyIddoc[$i]['doc_name'] . ' not found';
                                                }
                                                ?>
