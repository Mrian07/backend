...
$ord = ' ORDER BY dm.doc_masterid DESC';
if($sortby == 1){
  if($order == 0)
...
$sql = "SELECT COUNT(dm.doc_masterid) AS Total FROM `document_master` AS dm
        LEFT JOIN `country` AS c ON c.vCountryCode=dm.country
		WHERE dm.status != 'Deleted' $ssql";
...
$sql = "SELECT dm.doc_masterid, dm.doc_usertype , dm.doc_name, dm.status , c.vCountry FROM `document_master` AS dm
        LEFT JOIN `country` AS c ON c.vCountryCode=dm.country
        WHERE dm.status != 'Deleted' $ssql $ord LIMIT $start, $per_page";
                                            ...
                                            <table class="table table-striped table-bordered table-hover">
                                                <thead>
                                                    <tr>
														...
														<th width="25%"><a href="javascript:void(0);" onClick="Redirect(1,<?php if($sortby == '1'){ echo $order; }else { ?>0<?php } ?>)">Country <?php if ($sortby == 1) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
