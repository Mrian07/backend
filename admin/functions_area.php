<?
	...
	if($country_main_id != " "){
		$sql_country_main = "SELECT * FROM country where iCountryId=".$country_main_id." order by iCountryId desc";
	...
	if($country_id != " "){
		$sql_state = "SELECT * FROM state where iCountryId=".$country_id." order by iCountryId desc";
	...
	if($state_id != " "){
		$sql_city = "SELECT * FROM city where iStateId=".$state_id." order by iCityId desc";
	...
	if($country_id_not_required != " "  ){
		$sql_state = "SELECT * FROM state where iCountryId=".$country_id_not_required." order by iCountryId Asc";
		$db_data_state = $obj->MySQLSelect($sql_state);
		if(empty($db_data_state)){
			//echo "<option selected='selected' value=''>"; echo "No records for State."; echo "</option>";
		}
		else{
			echo "<option selected='selected' value=''>"; echo "Select any."; echo "</option>";
			for($i=0;$i<count($db_data_state);$i++){
				if($country_id_not_required){
	...
	if($state_id_not_required != " "){
		$sql_city = "SELECT * FROM city where iStateId=".$state_id_not_required." order by iCityId desc";
		$db_data_city = $obj->MySQLSelect($sql_city);
			echo "<option  value=''>"; echo "Select any."; echo "</option>";
			for($i=0;$i<count($db_data_city);$i++){
				if($state_id_not_required){
