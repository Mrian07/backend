...
$sql = "SELECT COUNT(iDriverId) AS Total FROM register_driver rd
		LEFT JOIN company c ON rd.iCompanyId = c.iCompanyId 
		WHERE rd.eStatus != 'Deleted' $ssql $dri_ssql";
...
$sql = "select iDriverVehicleId from driver_vehicle WHERE iDriverId='".$data_drv[$i]['iDriverId']."' AND eStatus!='Deleted'";
 ...
 $sql = "SELECT rd.iDriverId,rd.vEmail,rd.tRegistrationDate,rd.vPhone,rd.eStatus,(SELECT count(dv.iDriverVehicleId) FROM driver_vehicle AS dv WHERE dv.iDriverId=rd.iDriverId AND dv.eStatus != 'Deleted') AS `count`,CONCAT(rd.vName,' ',rd.vLastName) AS driverName, c.vCompany FROM register_driver rd 
		LEFT JOIN company c ON rd.iCompanyId = c.iCompanyId 
		WHERE rd.eStatus != 'Deleted' $ssql $dri_ssql $ord LIMIT $start, $per_page";
		// die;
                        ...
                        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="admin-nir-table">
                              <tbody>
                                <tr>
                                    ...
                                    <td width="30%"><a class="add-btn" href="driver_action.php" style="text-align: center;">Add Driver</a></td>
                                </tr>
                              </tbody>
                        </table>
                                            ...
                                            <table class="table table-striped table-bordered table-hover">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th width="13%"><a href="javascript:void(0);" onClick="Redirect(1,<?php if($sortby == '1'){ echo $order; }else { ?>0<?php } ?>)">Driver Name <?php if ($sortby == 1) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
														...
														<th width="18%"><a href="javascript:void(0);" onClick="Redirect(3,<?php if($sortby == '3'){ echo $order; }else { ?>0<?php } ?>)">Email <?php if ($sortby == 3) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
                                                        ...
                                                        <th width="12%" class="align-center">Edit Document</th>
                                                                ...
                                                                <div class="share-button openHoverAction-class" style="display: block;">
                                                                    ...
                                                                    <div class="social show-moreOptions for-five openPops_<?= $data_drv[$i]['iDriverId']; ?>">
                                                                        <ul>
																			...
																			<li class="entypo-gplus" data-network="gplus"><a href="javascript:void(0);" onClick="changeStatusDelete('<?php echo $data_drv[$i]['	iDriverId']; ?>')"  data-toggle="tooltip" title="Reset">
