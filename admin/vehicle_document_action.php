...
if ($action='document' && isset($_POST['doc_type'])) {
    ...
    if ($image_name != "") {
        ...
        if ($flag_error == 1) {
            //header("location:vehicle_document_fetch.php?success=1&id=" . $_REQUEST['id'] . "&var_msg=" . $var_msg);
            //$generalobj->getPostForm($_POST, $var_msg, "vehicle_document_fetch.php?success=1&id=" . $_REQUEST['id'] . "&var_msg=" . $var_msg);

            exit;
        } /* else if($_REQUEST['id'] != '') {
          header("location:vehicle_document_fetch.php?success=0&var_msg=something went wrong. Try again");
                ...
                <div class="inner">
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <a href="vehicles.php?type=<? echo $_REQUEST['type']?>">
                                            ...
                                            <div class="panel-body">
                                                <?php if ($db_userdoc[$i]['doc_file'] != '') { ?>
                                                    ...
                                                    if ($file_ext == 'is_image') {
                                                        ?>
                                                        <img src = "<?= $tconfig["tsite_upload_vehicle_doc_panel"] . '/' . $_REQUEST['id'] . '/' . $db_userdoc[$i]['doc_file'] ?>" style="width:200px;cursor:pointer;" alt ="YOUR DRIVING LICENCE" data-toggle="modal" data-target="#myModallicence"/>
                                                    <?php } else { ?>
                                                        <a href="<?= $tconfig["tsite_upload_vehicle_doc_panel"] . '/' . $_REQUEST['id'] . '/' . $db_userdoc[$i]['doc_file'] ?>" target="_blank"><?php echo $db_userdoc[$i]['doc_name']; ?></a>
                                                    <?php } ?>
                                                    <?php
                                                } else {
                                                    echo $db_userdoc[$i]['doc_name'] . ' not found';
                                                }
                                                ?>
