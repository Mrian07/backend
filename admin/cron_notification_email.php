...
function get_DriverDetail($id, $booking_date, $iUserId, $BookingNo) {
    ...
    $query = "SELECT vCode,vPhone FROM register_driver WHERE iDriverId=" . $id;
    ...
    $message_layout = send_messages_user("DRIVER_SEND_MESSAGE", $maildata);
...
function get_PassengerDetail($iUserId, $booking_date, $id, $BookingNo) {
    ...
    $query = "SELECT vPhone,vPhoneCode FROM register_user WHERE iUserId=" . $iUserId;
    ...
    $message_layout = send_messages_user("USER_SEND_MESSAGE", $maildata);
    return sendCode($vPhone, $vcode, $message_layout, "");
}
