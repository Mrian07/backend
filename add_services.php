        ...
        <div class="page-contant">
		    <div class="page-contant-inner page-trip-detail">
		      	<h2 class="header-page trip-detail driver-detail1"><?=$langage_lbl['LBL_MY_AVAILABILITY'];?>
					<? if($APP_TYPE == "UberX" && $_SESSION['sess_user'] == "company"){?>
						<a href="driverlist">
							<img src="assets/img/arrow-white.png" alt="">
							<?=$langage_lbl['LBL_BACK_ To_ Listing']; ?>
						</a>
					<? }?>
				</h2>
		      	<!-- trips detail page -->
				...
				<?} ?>
					<form name="frm1" method="post" action="">
		      				...
		      				<?php		      				
								...
								<fieldset>
								  ...
								  <?php foreach($value1['newData'] as $value) {
									...
									}else if($value['eFareType'] == 'Hourly'){
									$eFareType = 'Per hour'; 
									}else{
									$eFareType = '';
								  ...
								  ?>
									<li>
										...
										<div class="make-switch" data-on="success" data-off="warning">
											<input type="checkbox" <? if($chngamt == "Enabled"){ ?>onchange="check_box_value(this.value);" <? } ?> id="vCarType1_<?=$value['iVehicleTypeId'] ?>" class="chk" name="vCarType[]" <?php if(in_array($value['iVehicleTypeId'],$vCarTyp)){?>checked<?php } ?> value="<?=$value['iVehicleTypeId'] ?>" />
										</div>
										<?php 
										if($chngamt == "Enabled"){
											...
											?>
										<div class="hatchback-search" id="amt1_<?=$value['iVehicleTypeId'] ?>" <? echo $p001;?>>
											<input type="hidden" name="desc" id="desc_<?=$value['iVehicleTypeId']?>" value="<?=$value[$VehicleName1] ?>">
											<?php if($value['eFareType'] != 'Regular'){ ?>	
											<input class="form-control" type="text" name="fAmount[<?=$value['iVehicleTypeId']?>]" value="<?=$fAmount[$value['iVehicleTypeId']]?>" placeholder="Enter Amount for <?=$value[$VehicleName1] ?>" id="fAmount_<?=$value['iVehicleTypeId']?>"><label><?php echo $eFareType;?></label>
											</div>
										<? 	}
										}
										?>
						...
						<?php }?>
		      				...
		      				<strong><input type="submit" class="save-vehicle" name="submit1" id="submit1" value="Submit" <? if($chngamt == "Enabled"){ ?>onclick="return check_empty();"<? } ?>> </strong>
