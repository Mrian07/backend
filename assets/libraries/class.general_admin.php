<?php
	include_once("class.general_admin_ext.php");
	Class General_admin extends General_admin_ext
	{
		...
		public function getTripStates($tripStatus=NULL,$startDate="",$endDate="")
		{
			...
			if($tripStatus != "") {
				...
				$sql = "SELECT * FROM trips WHERE 1".$cmp_ssql.$ssl.$dsql;
				$data = $obj->MySQLSelect($sql);
			}
			return $data;
		}
		...
		public function getTransforAmountbyDriverId ($id,$ssql,$tip='')
		{
			...
			if($id != "") {
				...
				$sql = "SELECT IFNULL( SUM( IFNULL( fTripGenerateFare, 0 ) ) + SUM( IFNULL( fTipPrice, 0 ) ) , 0 ) - IFNULL( SUM( IFNULL( fCommision, 0 ) ) , 0 ) AS amounts FROM trips WHERE eDriverPaymentStatus = 'Unsettelled' AND vTripPaymentMode='Card' AND iDriverId = '".$id."'".$ssql;
				$data = $obj->MySQLSelect($sql);
