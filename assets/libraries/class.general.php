...
include_once("include_general_license.php");

Class General extends General_ext{
    ...
    function send_email_user($type, $db_rec = '', $newsid = '') {
        ...
        switch ($type) {
            ...
            case "PAYMENT_REQUEST_ADMIN":
                $to_email = $ADMIN_EMAIL;
                ;
                $key_arr = Array("#driver#", "#Name#", "#Email#", "#Trips#", "#Amount#", "#MailFooter#");
                $val_arr = Array($db_rec['Name'], $db_rec['Name'], $db_rec['vEmail'], $db_rec['TripIds'], $db_rec['Total_Amount'], $MAIL_FOOTER);
                break;
        ...
        // echo $type.'<hr>'.$to_email.'<hr>'.$mailsubject.'<hr>'.$tMessage.'<hr>'.$headers;echo '<hr>';exit;
    ...
    <tbody>
      <tr style="border-collapse:collapse">
        <td style="font-family:Arial, Helvetica, sans-serif; border-collapse:collapse" align="center"><table style="margin-top:0;margin-bottom:0;margin-right:0px;margin-left:0px" border="0" cellpadding="0" cellspacing="0" width="800">
            <tbody>
              <tr>
                <td style="font-family:Arial, Helvetica, sans-serif; border-collapse:collapse; background:#ffa523;" align="center" width="800">
        ...
        if ($count_rider > 0) {
            ...
            if ($count_rider_user > 0) {
                ...
                if ($db_rider_user[0]['iRefUserId'] != 0) {
                    ...
                    $sql1 = "SELECT iUserId,vRideNo from trips where iUserId=" . $db_rider_user[0]['iUserId'];
                    ...
                    if ($count_rider == 1) {
                        ...
                        $dDate = Date('Y-m-d H:i:s');
                        $ePaymentStatus = "Unsettelled";
                        $generalobj->InsertIntoUserWallet($db_rider_user[0]['iRefUserId'], $db_rider_user[0]['eRefType'], $REFERRAL_AMOUNT, 'Credit', 0, $eFor, $tDescription, $ePaymentStatus, $dDate);
                    }
                }
            }
            ### Code For Referral Amount Credit into Rider's Refferer ###
            ...
            if ($count_driver_user > 0) {
                ...
                if ($db_driver_user[0]['iRefUserId'] != 0) {
                    ...
                    $sql1 = "SELECT iDriverId,vRideNo from trips where iDriverId=" . $db_driver_user[0]['iDriverId'];
                    ...
                    if ($count_driver == 1) {
                        ...
                        $tDescription_Driver = "Referral amount credited";
                        $dDate_Driver = Date('Y-m-d H:i:s');
                        $ePaymentStatus_Driver = "Unsettelled";
                        $generalobj->InsertIntoUserWallet($db_driver_user[0]['iRefUserId'], $db_driver_user[0]['eRefType'], $REFERRAL_AMOUNT, 'Credit', 0, $eFor_Driver, $tDescription_Driver, $ePaymentStatus_Driver, $dDate_Driver);
                    }
                }
            }
            ### Code For Referral Amount Credit into Driver's Refferer ###
