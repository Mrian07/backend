...
if(isset($_REQUEST['countryId'])) {
	...
	if($fromMod == ""){
		...
		$sql = "select iCountryId from country where 1=1 AND vCountryCode='".$countryId."'";
	...
	if($countryId != ""){
		$sql = "select iStateId, vState from state where 1=1 $where";
...
if(isset($_REQUEST['stateId'])) {
	if($fromMod == "")
		...
		$cons = "<option value=''>Select</option>";
	if($stateId != ""){
		$sql = "select iCityId, vcity from city where iStateId = '".$stateId."'";
		$db_states = $obj->MySQLSelect($sql);
