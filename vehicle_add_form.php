<?php
	...
	if ($_SESSION['sess_user'] == 'company') {
		...
		$sql = "select c.iCountryId from `company` AS rd LEFT JOIN country AS cn ON c.vCountryCode=cn.vCountry where cn.iCompanyId = '" . $_SESSION['sess_iCompanyId'] . "'";
		$db_usr = $obj->MySQLSelect($sql);
		$iCountryId = $db_usr[0]['iCountryId'];
	}
	...
	if($Vehicle_type_name == "Ride-Delivery"){
		$vehicle_type_sql = "SELECT * from  vehicle_type where(eType ='Ride' or eType ='Deliver') AND (iCountryId='-1' OR iCountryId='".$iCountryId."')";
		$vehicle_type_data = $obj->MySQLSelect($vehicle_type_sql);
	}else{
		if($APP_TYPE == 'UberX'){
			$vehicle_type_sql = "SELECT vt.*,vc.* from  vehicle_type as vt  left join vehicle_category as vc on vt.iVehicleCategoryId = vc.iVehicleCategoryId where vt.eType='".$Vehicle_type_name."' AND (iCountryId='-1' OR iCountryId='".$iCountryId."')";
			$vehicle_type_data = $obj->MySQLSelect($vehicle_type_sql);
		}else{
			$vehicle_type_sql = "SELECT * from  vehicle_type  where eType='".$Vehicle_type_name."' AND (iCountryId='-1' OR iCountryId='".$iCountryId."')";
			$vehicle_type_data = $obj->MySQLSelect($vehicle_type_sql);
		}
	}
