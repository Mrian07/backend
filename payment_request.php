...
$sql = "SELECT t.*, t.iTripId,t.tSaddress, t.tEndDate,t.tDaddress,t.iFare,t.fCommision,t.ePayment_request, t.fWalletDebit FROM trips t
			      			...
			      			<form  name="frmbooking" id="frmbooking" method="post" action="payment_request_a.php">
			        			...
			        			<table width="100%" border="0" cellpadding="0" cellspacing="0" id="dataTables-example" <?php echo $rtls; ?>>
										...
										<? $fareTotal = $commTotal = $tipPriceTotal = $payTotal = 0;
											for($i=0;$i<count($db_dtrip);$i++) {
												...
												<tr class="gradeA">
													...
													<td ><?= $generalobj->DateTime1($db_dtrip[$i]['tEndDate'],'no');?></td>
													...
													<td><?=$db_dtrip[$i]['vTripPaymentMode']?></td>
													<td class="center"><a href="invoice.php?iTripId=<?php echo $db_dtrip[$i]['iTripId']?>"><img src="assets/img/driver-view-icon.png"></a>
													</td>
													<td>
									...
									<tr class="last_row_record">
										<td></td><td></td><td class="last_record_row"><?= $tripcursymbol;?> <?php echo /* $curr_code." ". */$generalobj->trip_currency_payment($fareTotal); ?></td><td class="last_record_row midddle_rw"><?= $tripcursymbol;?> <?php echo /* $curr_code." ". */$generalobj->trip_currency_payment($commTotal); ?></td>
										<?php if ($ENABLE_TIP_MODULE == "Yes") { ?><td class="last_record_row midddle_rw"><?= $tripcursymbol;?> <?php echo /* $curr_code." ". */$generalobj->trip_currency_payment($tipPriceTotal); ?></td><?php } ?><td class="last_record_row"> <?= $tripcursymbol;?><?php echo /* $curr_code." ". */$generalobj->trip_currency_payment($payTotal); ?></td><td></td><td></td>
									</tr>
									</tfoot>
		        				</table>
		        				<!--table>
