	...
	function sendRequest($cabId){
		...
		/*	
			...
			if(count($Data) > 0){
				...
				if(count($registation_ids_new) > 0){
					//Email to admin for assigned Driver
					/* $messageEmail = array();
					$messageEmail['details'] = '<p>Dear Administrator,</p>
								<p>Driver is assigned successfully for the following manual booking.</p>
								<p>Name: '.$passengerName.',</p>
								<p>Contact Number: +'.$vPhoneCode.$vPhone.'</p>';
					$mail = $generalobj->send_email_user('CRON_BOOKING_EMAIL',$messageEmail);
					//Email to admin for assigned Driver
					$where_cabid2 = " iCabBookingId = '".$data_booking[0]['iCabBookingId']."'";
				...
				if(count($deviceTokens_arr_ios) > 0){
					...
					$Data_update2['eAssigned']= 'Yes';
					$id = $obj->MySQLQueryPerform("cab_booking",$Data_update2,'update',$where_cabid2); 
					$result = sendApplePushNotification(1,$deviceTokens_arr_ios,$message,$alertMsg,0);
				}
			}else{
				//Email to admin for Not assigned Driver
				$message = array();
